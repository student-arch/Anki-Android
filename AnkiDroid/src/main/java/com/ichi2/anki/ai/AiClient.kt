...
open class AiClient(
    ...
    /**
     * Performs a **streaming** chat completion: [onDelta] receives the accumulated message
     * text after every Server-Sent-Events chunk, so callers can parse and display partial
     * results (e.g. each flashcard) before the response completes.
     *
     * The payload is [chatCompletion]'s with `stream: true`; reasoning-model `reasoning_content`
     * deltas are skipped. Providers that reject streaming fall back to the batch request, so
     * [onDelta] is still invoked once with the full content.
     *
     * @return the final accumulated message text
     * @throws AiException if the request fails or the response cannot be parsed
     */
    open suspend fun chatCompletionStream(
        provider: AiProvider,
        modelId: String,
        systemPrompt: String,
        userPrompt: String,
            ...
            withContext(Dispatchers.IO) {
                val response =
                    try {
                        ...
                        Timber.w(e, "AI request failed (streaming) for %s", provider.name)
                        throw AiException.Network(e.message ?: "network error", e)
                    }
                response.use {
                    if (!it.isSuccessful) {
                        Timber.w("AI request failed (streaming): HTTP %d", it.code)
                        throw httpError(it.code, it.body.string())
                    }
                    readSseStream(it.body.byteStream(), onDelta)
                }
            }
        } catch (e: AiException.BadRequest) {
            if (!jsonMode) throw e
            // as in the batch path: retry without response_format when unsupported
            Timber.i("json_mode rejected by provider, retrying without it")
            chatCompletionStream(provider, modelId, systemPrompt, userPrompt, jsonMode = false, onDelta = onDelta)
        }
    }

    /**
     * Reads an SSE `chat/completions` stream, accumulating `delta.content` and reporting each
     * snapshot to [onDelta]. Non-`data:` lines, `delta.reasoning_content` (reasoning models)
